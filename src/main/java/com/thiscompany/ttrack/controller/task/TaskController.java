...
public class TaskController {
    ...
    public ResponseEntity<TaskResponse> findTaskById(
        @NotEmpty(message = "field.id.is_empty") @PathVariable("id") String id,
        @AuthenticationPrincipal User user
    ) {
        return ResponseEntity.ok(taskService.findTaskById(id, user.getUsername()));
    }
    ...
    public ResponseEntity<List<TaskResponse>> findAllTasks(@AuthenticationPrincipal User user) {
        return ResponseEntity.ok(taskService.findAllUserTasks(user.getUsername()));
    }
    ...
    @AuthenticationPrincipal User user) {
        return new ResponseEntity<>(
            taskService.createTask(request, (User) user),
            HttpStatus.CREATED
        );
    }
    ...
    public ResponseEntity<Page<TaskResponse>> findTasksWithParams(
        ...
        return ResponseEntity
                .ok(taskService.findTasksByParams(
                    user.getUsername(), title, details, status, state, priority, isCompleted, pageable)
                );
    }
